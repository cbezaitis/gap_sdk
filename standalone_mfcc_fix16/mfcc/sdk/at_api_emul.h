...
do { \
  ...
  memcpy(__To, __From, (size_t)(size)); \
} while (0)
...
do { \
  ...
  size_t __Chunk, __Len = (length); \
  for (__Chunk=0; __Chunk<(size); __Chunk+=__Len) { \
    memcpy(__To, __From, __Len); \
    if (CopyIn) { __From += (stride); __To += __Len; } \
    else        { __To += (stride); __From += __Len; } \
  } \
} while (0)
